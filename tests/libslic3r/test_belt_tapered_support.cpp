...
double support_reference_y(const Model& model, double tangent)
{
        ...
        for (const ModelInstance* instance : object->instances) {
            for (const ModelVolume* volume : object->volumes) {
                ...
                for (const Vec3f& vertex : volume->mesh().its.vertices) {
                    ...
                    reference = std::max(reference, world.y() + std::max(0.0, world.z()) * tangent);
                }
            }
        }
    }
...
TEST_CASE("Belt contact projects to the world build plate", "[belt][support]")
{
    ...
    const double root_s = -contact.y() / belt.cot_angle();
    const Vec3d root_world = belt.oriented_to_world({contact.x(), contact.y(), root_s});

    CHECK(root_world.x() == Catch::Approx(contact_world.x()));
    CHECK(root_world.y() == Catch::Approx(contact_world.y() + contact_world.z()));
    CHECK(root_world.z() == Catch::Approx(0.0).margin(1e-9));
    CHECK(contact.y() == Catch::Approx(belt.belt_boundary_v(root_s)).margin(1e-9));
}
          ...
          "[belt][support][integration]")
{
    ...
    const double reference_y = support_reference_y(model, 1.0);
    ...
    CHECK(final_belt->print_origin_s() > 0.0);
