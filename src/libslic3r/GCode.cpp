...
std::vector<GCode::LayerToPrint> GCode::collect_layers_to_print(const PrintObject& object)
{
    ...
    while (idx_object_layer < object.layers().size() || idx_support_layer < object.support_layers().size()) {
        ...
        if (layers_to_print.size() == 1u) {
            if (!has_extrusions && object.print()->is_belt_printer()) {
                // An oblique plane starts at the shared belt reference P0. A model may
                // therefore have valid geometric layers which are still too narrow to
                // produce an extrusion. They are leading sampling layers, not gaps
                // underneath a later extrusion, so omit them from G-code ordering.
                layers_to_print.pop_back();
                continue;
            }
            if (!has_extrusions)
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    m_writer.set_belt_coordinate_system(print.belt_coordinate_system());
    ...
    if(!has_BTT_thumbnail){   
        ...
        if (const BeltCoordinateSystem *belt = print.belt_coordinate_system()) {
            file.write(";SLICING_KINEMATICS:BELT\n");
            file.write(";BELT_COORDINATE_VERSION:2\n");
            file.write_format(";BELT_GANTRY_ANGLE:%.9f\n", belt->angle_degrees());
            file.write_format(";BELT_PLATE_MAX_WORLD_Y:%.9f\n", belt->plate_max_world_y());
            file.write_format(";BELT_PRINT_ORIGIN_S:%.9f\n", belt->print_origin_s());
        }
        if (is_bbl_printers)
    ...
    const bool                               prime_extruder)
{
    ...
    // export layer z
    char buf[64];
    sprintf(buf, (print.is_BBL_printer() || print.is_belt_printer())
                     ? "; Z_HEIGHT: %g\n"
                     : ";Z:%g\n",
            print_z);
    gcode += buf;
