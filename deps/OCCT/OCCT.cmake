...
# A host application may provide the OCCT package used by its own geometry
# code, avoiding incompatible DLLs with identical names in the same process.
set(LIBSLICER_EXTERNAL_OCCT_DIR "" CACHE PATH "Host-provided OpenCASCADE package directory")
if(LIBSLICER_EXTERNAL_OCCT_DIR)
    if(NOT EXISTS "${LIBSLICER_EXTERNAL_OCCT_DIR}/OpenCASCADEConfig.cmake")
        message(FATAL_ERROR "Invalid host OCCT package: ${LIBSLICER_EXTERNAL_OCCT_DIR}")
    endif()
    set(OpenCASCADE_DIR "${LIBSLICER_EXTERNAL_OCCT_DIR}" CACHE PATH "OpenCASCADE config dir" FORCE)
    set(OCCT_AVAILABLE TRUE CACHE BOOL "OCCT library is available" FORCE)
    return()
endif()
...
if(OCCT_FOUND)
    ...
    set(_occt_package_dir "")
    foreach(_occt_layout lib/cmake/occt lib/cmake/opencascade cmake)
        if(EXISTS "${OCCT_INSTALL_DIR}/${_occt_layout}/OpenCASCADEConfig.cmake")
            set(_occt_package_dir "${OCCT_INSTALL_DIR}/${_occt_layout}")
            break()
        endif()
    endforeach()
    if(NOT _occt_package_dir)
        message(FATAL_ERROR "OCCT package configuration missing from ${OCCT_INSTALL_DIR}")
    endif()
    set(OpenCASCADE_DIR "${_occt_package_dir}" CACHE PATH "OpenCASCADE config dir" FORCE)
